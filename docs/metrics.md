...
(always on; auto-reverts to vanilla permanently if it ever throws). Vanilla walks every connection and player slot per zombie and reaches
...
time via `CounterWithCallback`.

| Name | Type | Labels | What |
|------|------|--------|------|
| `pz_zombie_auth_scan_passes_total` | CounterWithCallback | `path={optimized,vanilla}` | Packer auth passes by executed path (`vanilla` = failure latch, rotate-ownership debug option, or no engine yet). |
...
| `storm_server_cell_recalc_failures_total` | Counter | — | Throwables swallowed by the `ServerCell.RecalcAll2` crash guard (`ServerCellRecalcCrashGuardPatch`). Vanilla retries a throwing cell from `ServerMap.preupdate` every frame forever, freezing the world loop while network threads keep serving; the guard drops the cell from the retry lists instead, so each increment is a cell that finished loading partially recalculated. The swallowed stack trace is in the server log at ERROR — any increase deserves a look, because whatever corrupted the entity registry is still latent. `VirtualMachineError`s are never swallowed and never counted. |
| `storm_glass_attachment_removal_failures_total` | Counter | — | Objects `IsoGridSquare.removeGlassAttachments` asked `RemoveTileObject` to drop from a smashed window's square that stayed on the square (`IsoGridSquareRemoveGlassAttachmentsPatch`). Vanilla re-examines such an object forever and pins the main thread at 0 TPS; the guard skips it and logs the object and square at WARN. Each increment is a multi-tile fixture (typically a two-sprite light switch) whose other part could not be resolved — the fixture stays on the square, playable but not removed. |
| `storm_pathfind_chunk_tasks_drained_total` | Counter | — | Native pathfinder chunk tasks (chunk updates and removes) executed by Storm's per-frame drain ahead of vanilla's ten updates per frame (`PathfindChunkTaskDrainPatch`). Tracks the server's chunk add rate; compare with `jvm_buffer_pool_used_buffers{pool="direct"}`, which climbs without bound when the queue is backing up, since every queued task holds one direct buffer. |
| `storm_pathfind_chunk_task_budget_exhausted_total` | Counter | — | Pathfinder frames whose drain spent `-Dstorm.pathfind.chunkTaskBudgetMs` with chunk tasks still queued. Occasional increments are bursts; a steady climb means the queue is backing up again and the budget wants raising. |
