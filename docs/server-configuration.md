...
| `Storm.ZombieAuthFastPath` | `true` | boolean | Snapshot-backed fast path for `NetworkZombieManager.updateAuth(IsoZombie)` — the per-zombie ownership scan, the only server-tick cost that grows with players × zombies (6.7% of the main thread at 112 players / 7,000 zombies). Storm flattens the (connection, player) rows once per packer pass and scans the rows with a single ECS component fetch per zombie; the 2 s gate, grapple/target early-outs, owner-switch hysteresis, reanimated-corpse sweep, and final relevance check are outcome-identical, and the rotate-ownership debug option routes to vanilla. Composes with `Storm.ZombieAuthTickInterval`. Set `false` to restore the vanilla scan (live-appliable via admin sandbox push); the fast path also auto-reverts permanently if it ever throws. |
