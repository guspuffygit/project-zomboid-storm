...
| `-Dstorm.reapStalledConnectionMs=<ms>` | Idle window before a connection that stalled mid-handshake is dropped and its RakNet slot freed. Default `420000` (7 minutes) — **on by default**, this is half of the "Getting Server Info..." fix. Only ever applies to connections that never reached `isFullyConnected()` and have sent no inbound game packet for the whole window; fully-connected players are never touched. Raise it if your players routinely need longer than 7 minutes between login and spawning in. |
| `-Dstorm.raknet.connectionHeadroom=<n>` | Spare RakNet incoming-connection slots above `MaxPlayers`, for the login pipeline. Default `64`. The resolved cap is `clamp(max(101, MaxPlayers + n), .., 256)`, so the vanilla 101 is a floor (small servers are never regressed) and 256 is a hard ceiling. |
