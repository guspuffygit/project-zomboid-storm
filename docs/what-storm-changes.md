...
- **Net-data drain cap** — `GameServer.mainLoopDealWithNetData` (HIGH + player-update + vehicle inbound drain) gets a per-outer-loop-spin wall-clock cap from `Storm.NetDataCapMs` (default 90 ms; `0` disables). When the cap fires, subsequent packets in the same spin are short-circuited and the next spin starts fresh — RakNet retransmits any reliable ones. Protects world-tick scheduling during reconnect storms; deferrals counted by `pz_netdata_deferred_total`.
