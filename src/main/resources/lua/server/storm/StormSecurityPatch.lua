...
Commands.remove = function(player, args)
    if isAdmin(player)
            or player:getRole():hasCapability(Capability.ManipulateVehicle) then
        return _origVehicleRemove(player, args)
    end
    print(string.format(
        "[StormSecurityPatch] BLOCKED vehicle.remove from %s vehicle=%s",
        player:getUsername(), tostring(args and args.vehicle)))
end
...
Commands.player.onHealthCheatCurrentPlayer = function(player, args)
    if (args and args.id == player:getOnlineID())
            or checkPermissions(player, Capability.UseHealthCheat) then
