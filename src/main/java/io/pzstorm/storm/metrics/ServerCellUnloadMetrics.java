...
public final class ServerCellUnloadMetrics {
    ...
    static {
        Thread reporter =
                new Thread(ServerCellUnloadMetrics::reporterLoop, "StormServerCellUnloadMetrics");
    ...
    private static void reporterLoop() {
        while (true) {
            ...
            try {
                ...
                StormLogger.LOGGER.warn("ServerCellUnloadMetrics reporter failed", t);
            }
        }
    }
    ...
    private static void report() {
        ...
        if (ticks == 0L && calls == 0L) {
            StormLogger.LOGGER.info(
                    "ServerCellUnloadMetrics: window={}ms ticks=0 calls=0 (no activity)", windowMs);
            return;
        }
        ...
        StormLogger.LOGGER.info(
                "ServerCellUnloadMetrics: window={}ms ticks={} calls={} totalMs={} avgPerTickMs={}"
