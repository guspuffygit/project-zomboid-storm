...
/**
 * Accumulates per-tick CPU time spent in {@code IsoAnimal.update()} on the main thread and reports
 * a rolling 60-second average.
 *
 * <p>Wired by:
 *
 * <ul>
 *   <li>{@code IsoAnimalUpdateTimingAdvice} &mdash; calls {@link #recordUpdateNanos(long)} for each
 *       {@code IsoAnimal.update()} invocation on the server main thread.
...
public final class AnimalUpdateMetrics {
    ...
    static {
        Thread reporter = new Thread(AnimalUpdateMetrics::reporterLoop, "StormAnimalMetrics");
    ...
    /** Called from advice on the server main thread for every {@code IsoAnimal.update()} call. */
    ...
    private static void reporterLoop() {
        while (true) {
            ...
            try {
                ...
                StormLogger.LOGGER.warn("AnimalUpdateMetrics reporter failed", t);
            }
        }
    }
    ...
    private static void report() {
        ...
        if (ticks == 0L && calls == 0L) {
            StormLogger.LOGGER.info(
                    "AnimalUpdateMetrics: window={}ms ticks=0 calls=0 (no activity)", windowMs);
            return;
        }
        ...
        StormLogger.LOGGER.info(
                "AnimalUpdateMetrics: window={}ms ticks={} calls={} totalMs={} avgPerTickMs={}"
