...
public final class NetDataMetrics {
    ...
    static {
        Thread reporter = new Thread(NetDataMetrics::reporterLoop, "StormNetDataMetrics");
    ...
    private static void reporterLoop() {
        while (true) {
            ...
            try {
                ...
                StormLogger.LOGGER.warn("NetDataMetrics reporter failed", t);
            }
        }
    }
    ...
    private static void report() {
        ...
        if (ticks == 0L && calls == 0L) {
            StormLogger.LOGGER.info(
                    "NetDataMetrics: window={}ms ticks=0 calls=0 (no activity)", windowMs);
            return;
        }
        ...
        StormLogger.LOGGER.info(
                "NetDataMetrics: window={}ms ticks={} calls={} totalMs={} avgPerTickMs={}"
