 ...
 * still sitting in {@code ccrWaiting}, unanswered. The client has no resend timer; re-asks are
 * driven by the server's own ChunkNotReady replies or by the chunk map re-wanting a cancelled
 * chunk. That makes this the closest thing to client-side stall telemetry that can be read without
