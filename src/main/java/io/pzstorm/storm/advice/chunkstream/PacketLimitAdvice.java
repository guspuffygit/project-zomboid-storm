 ...
 * into {@code sentRequests}, so the client waits out its flat 8-second resend timer for chunks it
 * never asked for. The server's own call site only logs the result and sends anyway, which is why
 * this is gated on {@code GameClient.client}: it isolates the call that actually drops.
 *
