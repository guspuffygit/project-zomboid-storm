 ...
 * component}}, filled front-to-back and scanned linearly by requested class (the earlier
 * identity-hash slot addressing collided per boot — see {@code EcsTryGetComponentMemoAdvice}).
 * Pairs are immutable once published, so a hit is a few reference compares plus an ownership
 * re-validation ({@code component.getECSOwnerEntity() == entity} ⟺ the component is still
 * registered on the entity — {@code removeECSComponent} nulls the owner, {@code setECSComponent}
 * re-stamps it), and the memo can never return a component the vanilla map no longer holds. The
 * field lives only on {@code IsoGameCharacter} — not on {@code IsoObject}, which also implements
 * {@code ECSEntity} but exists in the millions; non-character entities fall through to the vanilla
 * lookup via an {@code instanceof} check in the advice.
 *
