...
public class StormClassLoader extends ClassLoader {
    ...
    /**
     * {@code Set} of class prefixes that mark classes to not load with this {@code ClassLoader}. If
     * a class matches this patter it's loading will be delegated to the parent loader. Classes
     * matching this prefix are causing exceptions when loaded with this {@code ClassLoader}, and
     * they also do not need to be loaded with this {@code ClassLoader}.
     */
    private static final ImmutableSet<String> CLASS_BLACKLIST =
            ImmutableSet.of(
                    ...
                    "io.pzstorm.storm.core.StormBootstrap",
                    "io.pzstorm.storm.core.StormClassLoader");

    /**
     * {@code ClassLoader} that is the parent of this {@code ClassLoader}. When loading classes,
     * those classes not matching the whitelist pattern will have their loading process delegated to
     * this {@code ClassLoader}.
     */
    ...
    public StormClassLoader() {
        LOGGER.debug("Initialized StormClassLoader");
        parentClassLoader = getClass().getClassLoader();
        modResourceLoader = new URLClassLoader("modResourceLoader", new URL[0], getParent());
    }

    /**
     * Returns {@code true} if at least one prefix pattern in blacklist matches the given name. When
     * a class name is considered <i>blacklisted</i> it will not be loaded by this {@code
     * ClassLoader}.
     */
    @Contract(pure = true)
     ...
     * @throws IllegalArgumentException if package name duplicates an existing package either in
    ...
    /**
     * Loads the class with the specified binary name. This method will get called every time a
     * class is about to be loaded by this {@code ClassLoader}. It is called before the class is
     * defined or loaded into JVM, which allows us to manipulate the class to our desire.
     *
    ...
    private Class<?> loadClassInternal(String name, boolean resolve) throws ClassNotFoundException {
            ...
            if (!isBlacklistedClass(name)) {
                ...
                try {
                    ...
                    if (StormBootstrap.hasLoaded()) {
                        input = StormBootstrap.applyAllTransformers(name, input);
                    }
                    if (input.length > 0) {
                        ...
                        if (clazz.getClassLoader() == this) {
                            LOGGER.trace("Successfully loaded {} with StormClassLoader", name);
                        } else
                            throw new RuntimeException(
                                    "Unable to load class with StormClassLoader");
                    }
        ...
        if (clazz == null) {
            LOGGER.trace("Loading with application class loader");
     ...
     * @return {@code byte} array read from {@code Class} or an empty array if the {@code Class}
