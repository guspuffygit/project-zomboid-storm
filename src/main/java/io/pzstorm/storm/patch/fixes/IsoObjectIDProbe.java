 ...
 * ZombieConfig.ZombiesCountBeforeDelete} sandbox option set to {@code 0}), the count of live
 * entries can grow into the same order of magnitude as the address space and collisions on the
 * {@code put(short, T)} silently overwrite an existing live entry, leaving its holder reachable in
 * {@code cell.zombieList} but unreachable by ID. The downstream symptom is client-side zombie
 * duplication ("mitosis") when the server starts broadcasting that overwritten holder under a
 * freshly allocated ID and the client's {@code IDToZombieMap.get(id)} miss path spawns a brand-new
 * client zombie via {@code createRealZombieAlways}.
 *
