...
class AsmUtilsTest implements UnitTest {
    ...
    private static final LabelNode[] LABELS =
            new LabelNode[] {
                new LabelNode(), new LabelNode(), new LabelNode(), new LabelNode(), new LabelNode()
            };
    ...
    void shouldAddInstructionsToTheEndOfInsnList() {
        AbstractInsnNode[] nodesToAdd =
                new AbstractInsnNode[] {
                    new LabelNode(),
                    new VarInsnNode(Opcodes.ALOAD, 0),
                    new MethodInsnNode(
                            Opcodes.INVOKESPECIAL, "io.pzstorm.storm.util", "<init>", "()V"),
        ...
        while (iteratorExpected.hasNext()) {
            Assertions.assertTrue(
                    AsmUtils.equalNodes(iteratorExpected.next(), iteratorActual.next()));
        }
    }
    ...
    void shouldGetNthLabelNodeFromInsnList() {
        ...
        Assertions.assertThrows(
                IllegalArgumentException.class, () -> AsmUtils.getNthLabelNode(instructions, -1));
        Assertions.assertThrows(
                IllegalArgumentException.class, () -> AsmUtils.getNthLabelNode(instructions, 0));
    ...
    void shouldGetLastInstructionOfType() {
        Map<AbstractInsnNode, AbstractInsnNode> instructionsOfType =
                ImmutableMap.of(
                        LABELS[4],
                        AsmUtils.getLastInstructionOfType(instructions, LabelNode.class),
                        LAST_METHOD_NODE,
                        AsmUtils.getLastInstructionOfType(instructions, MethodInsnNode.class),
                        LAST_LINE_NODE,
                        AsmUtils.getLastInstructionOfType(instructions, LineNumberNode.class),
                        LAST_VAR_NODE,
                        AsmUtils.getLastInstructionOfType(instructions, VarInsnNode.class));
    ...
    void shouldGetFirstMatchingLabelNodeInstruction() {
        ...
        for (int i1 = 0; i1 < labelIndexPairs.size(); i1++) {
            ...
            LabelNode actualLabel =
                    AsmUtils.getFirstMatchingLabelNode(createInstructionList(), insnToMatch);
    ...
    void shouldReplaceFirstMatchingLabelNodeInstructions() {
        ...
        List<AbstractInsnNode> expectedList =
                ImmutableList.of(
        ...
        InsnList instructions = createInstructionList();
        Assertions.assertTrue(
                AsmUtils.replaceFirstMatchingLabelNode(instructions, LABELS[1], expectedList));
