...
public class AsmUtils {
    ...
    public static @Nullable <T extends AbstractInsnNode> T getLastInstructionOfType(
            InsnList list, Class<T> node) {
     ...
     * @param n index of the node to retrieve from list. The value of the index has to larger then
     *     {@code 0} otherwise an exception will be thrown.
     * @return {@code LabelNode} found under the specified index or {@code null} if no {@code
     *     LabelNode} for that index was found.
    ...
    public static AbstractInsnNode nextNonLabelLineNumberNode(AbstractInsnNode node) {
        ...
        if (node.getNext() instanceof LabelNode
                || node.getNext() instanceof LineNumberNode
                || node.getNext() instanceof FrameNode) {
            return nextNonLabelLineNumberNode(node.getNext());
        }
        return node.getNext();
    }

    public static List<AbstractInsnNode> listWithoutLabelAndLineNumberNodes(
    ...
    /**
     * Find and return first {@link LabelNode} that contains given list of instructions. Note that
     * {@link LineNumberNode} and {@link LabelNode} instructions will be excluded when comparing
     * entries which means that these instructions are safe to be included in {@code match} list.
     *
     * @param list list of instructions to search.
     ...
     */
    public static @Nullable LabelNode getFirstMatchingLabelNode(
            InsnList list, List<AbstractInsnNode> match) {
        ...
        for (AbstractInsnNode instruction : list.toArray()) {
            if (instruction instanceof LabelNode) {
                ...
                if (firstNonLabelInstruction != null
                        && AsmUtils.equalNodes(firstNonLabelInstruction, matchList.get(0))) {
                    if (matches(
                            firstNonLabelInstruction.getNext(),
                            matchList.subList(1, matchList.size()))) {
    ...
    /**
     * Replace all instructions between the given {@link LabelNode} and next label node in specified
     * list of instructions. Note that label nodes will not be altered in any way, they are only
     * used to mark the start and end of replacement operation.
     *
     ...
     */
    // @formatter:off
    public static boolean replaceFirstMatchingLabelNode(
            InsnList list, LabelNode target, List<AbstractInsnNode> content) {
    ...
    /**
     * Compare the given nodes and return {@code true} if they are equal. Since instances of {@link
     * AbstractInsnNode} do not implement a equality comparison, comparing two node instances will
     * compare object addresses and omit comparing fields. This method does a deep comparison,
    ...
    public static boolean equalNodes(AbstractInsnNode a, AbstractInsnNode b) {
        ...
        if (a instanceof InvokeDynamicInsnNode) {
            return ((InvokeDynamicInsnNode) a).name.equals(((InvokeDynamicInsnNode) b).name)
                    ...
                    && Arrays.equals(
                            ((InvokeDynamicInsnNode) a).bsmArgs,
                            ((InvokeDynamicInsnNode) b).bsmArgs);
        }
        ...
        if (a instanceof LookupSwitchInsnNode) {
            return (AsmUtils.equalNodes(
                            ((LookupSwitchInsnNode) a).dflt, ((LookupSwitchInsnNode) b).dflt)
        ...
        if (a instanceof TableSwitchInsnNode) {
            ...
            if (!AsmUtils.equalNodes(
                    ((TableSwitchInsnNode) a).dflt, ((TableSwitchInsnNode) b).dflt)) {
                return false;
            }
