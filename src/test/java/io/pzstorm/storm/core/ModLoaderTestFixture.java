...
class ModLoaderTestFixture implements IntegrationTest {
    ...
    static final ImmutableMap<String, String[]> MOD_JAR_DATA =
            ImmutableMap.of(
    ...
    static void prepareTestClass(File tempDir) throws IOException {
        if (tempDir.exists()) {
            ...
            MoreFiles.deleteDirectoryContents(
                    tempDir.toPath(), RecursiveDeleteOption.ALLOW_INSECURE);
        ...
        for (Map.Entry<String, String[]> entry : MOD_JAR_DATA.entrySet()) {
            ...
            for (String modJarName : entry.getValue()) {
                ...
                try {
                    jarResource =
                            Objects.requireNonNull(CL.getResource("./jars/" + modJarName)).toURI();
    ...
    static void createAndWriteMetadataFiles(File tempDir) throws IOException {
        for (Map.Entry<String, String[]> entry : MOD_JAR_DATA.entrySet()) {
            ...
            writeToModMetadataFile(
                    new File(zomboidModsDir, modDirName),
