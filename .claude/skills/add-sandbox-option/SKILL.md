...
| `Storm.ImportantAreasMaximum` | `ImportantAreasPolicy#setMaximum` | Cap on the engine's `ImportantAreaManager` list (cells kept loaded for a lit stove or a running/alarming vehicle with nobody near). Vanilla inlines 100 and evicts at random; `ImportantAreasCapPatch` body-replaces `updateOrAdd` on the server so the cap is this option and the victim is the least-recently-refreshed entry. Range 100–2048, default 100 = vanilla's cap. Read per call, so live-appliable trivially. Gauge `storm_important_areas_maximum`; `pz_important_areas_size` / `pz_important_area_evictions_total` show whether it binds. |
